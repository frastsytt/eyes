from flask import request, Blueprint, render_template, redirect, url_for, session, jsonify, g, render_template_string
from . import db 
...
def admin():
    ...
    return render_template('admin.html', username=username, title="Admin", warning=warning, admin_role=user_role, user_email=email)
...
def review_news():
    ...
    existing_user = WebUser.query.filter_by(email=email).first()
    return render_template('adm_review_news.html', username=existing_user.username, admin_role=user_role, title="Review", stories=stories, user_email=email, deleted=deleted, error=error, warning=warning)
...
def create_approved_story_view(story_id):
    ...
    if request.method == 'GET':
        return render_template('adm_create_news_from_story.html', username=existing_user.username, admin_role=user_role, title="Create News", story=story, user_email=email, warning=warning)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('adm_create_news_from_story.html', username=existing_user.username, admin_role=user_role, title="Create News", story=story, user_email=email, warning=warning, error=1)
...
def view_news():
    ...
    stories = NewsStory.query.order_by(NewsStory.submission_date.desc()).paginate(page=page, per_page=10, error_out=False)
    return render_template('adm_view_news.html', admin_role=user_role, title="Create News",stories=stories, warning=warning, user_email=email)
...
def new_story():
    ...
    if request.method == 'GET':
        return render_template('adm_create_news.html', username=existing_user.username, admin_role=user_role, title="Create News", warning=warning, user_email=email)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('adm_create_news.html', username=existing_user.username, admin_role=user_role, title="Create News", warning=warning, user_email=email, error=1)
